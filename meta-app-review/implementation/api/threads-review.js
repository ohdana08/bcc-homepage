...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(503).send(page({ error: { message: error.message } }));
  }
  ...
  if (!session?.token) {
    return res.status(200).send(page({
      query,
      error: query ? { message: '먼저 OAuth 연결을 완료해 주세요.' } : null,
    }));
  }
