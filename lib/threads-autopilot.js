      ...
      items: {
        ...
        properties: {
          ...
          text_lines: {
            ...
            minItems: 5,
            maxItems: 10,
            items: { type: 'string', minLength: 1, maxLength: MAX_CONTENT_LINE_LENGTH },
          },
...
function orderedDailyPosts(result) {
  ...
  const normalized = result.posts.map((post) => ({
    ...
    text: formatBodyLines(post.text_lines),
    self_comment_0: post.self_comment_0_lines.join('\n'),
    self_comment_6h: post.self_comment_6h_lines.join('\n'),
