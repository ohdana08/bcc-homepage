...
const JIWONFIT_MAX_LINE_LENGTH = 60;
...
JSON 스키마 외의 말은 출력하지 않는다.`;
  ...
  if (config?.id !== 'jiwonfit') return basePrompt;

  return `${basePrompt}

딱, 지원핏의 검증된 글 리듬:
- 이 계정에서 실제 반응을 만든 작성자 고유의 형식을 우선한다. 문장을 베끼지 말고 구조와 리듬만 쓴다.
- 첫 줄은 독자의 기존 행동을 뒤집는 단호한 관점 전환으로 쓴다. 다음 이유가 궁금해져야 한다.
- 이어서 3~5개의 번호 항목으로 이유, 판단 기준, 실제 손해 또는 다음 행동을 전개한다.
- 번호 항목은 한 항목에 한 논리만 담은 자연스러운 완결 문장으로 쓴다. 억지로 10자 안에 자르지 않는다.
- 마지막은 독자가 기억할 한 문장 결론 또는 맥락 있는 질문으로 끝낸다. 모든 글에 질문 CTA를 억지로 붙이지 않는다.
...
export function normalizeShortLines(lines, {
  ...
  let normalized = (lines || []).flatMap((line) => wrapShortLine(line));
  if (finalQuestion && normalized.length) {
    ...
    if (last.length >= MAX_CONTENT_LINE_LENGTH) {
      ...
      while (words.length > 1 && `${words.join(' ')}?`.length > MAX_CONTENT_LINE_LENGTH) {
        words.pop();
      }
      last = words.join(' ');
      if (`${last}?`.length > MAX_CONTENT_LINE_LENGTH) {
...
export function ensureNumberedItems(lines, minimum = 2) {
  ...
  for (let index = 0; index < numbered.length && count < minimum; index += 1) {
    ...
    const available = MAX_CONTENT_LINE_LENGTH - prefix.length;
...
function orderedDailyPosts(result) {
  ...
  const normalized = result.posts.map((post) => {
    const textLines = normalizeShortLines(post.text_lines, { finalQuestion: true });
    let zeroLines = normalizeShortLines(post.self_comment_0_lines);
    if (['tip', 'template'].includes(post.content_type)) {
      zeroLines = ensureNumberedItems(zeroLines);
    }
    const sixHourLines = normalizeShortLines(post.self_comment_6h_lines);
    return {
...
export function validateHumanVoiceBatch(posts, customerPatterns = CUSTOMER_LANGUAGE_PATTERNS, options = {}) {
  ...
  posts.forEach((post, index) => {
    ...
    if (paragraphs.length < writingProfile.minParagraphs) problems.push(`${index + 1}번 본문이 ${writingProfile.minParagraphs}문단 미만`);
    ...
    if (writingProfile.requireNumberedStructure && !/\n\s*1[.)]\s+/.test(text)) {
      problems.push(`${index + 1}번 본문에 번호 논리 전개가 없음`);
    }
  });
...
function campaignWritingProfile(config) {
  if (config?.id === 'jiwonfit') {
    return {
      maxLineLength: JIWONFIT_MAX_LINE_LENGTH,
...
async function generateDailyBatch(client, model, config, dateKey, performanceContext) {
  const outputShape = config?.id === 'jiwonfit'
    ? `각 글은 줄바꿈된 문자열 대신 아래 배열 필드로 작성한다.
- text_lines: 5~9개. 첫 항목은 관점 전환 후크, 중간에 1., 2., 3. 번호 논리를 넣고 마지막은 결론 또는 질문이다.
- self_comment_0_lines: 3~8개.
- self_comment_6h_lines: 3~8개.
모든 항목은 60자 이내의 자연스러운 완결 문장으로 쓴다. 항목에는 빈 문자열이나 줄바꿈을 넣지 않는다.`
    : `${outputShape}`;
...
각 글은 줄바꿈된 문자열 대신 아래 배열 필드로 작성한다.
- text_lines: 5~10개. 첫 항목은 10자 이내 후크, 마지막 항목은 질문형 CTA 하나다.
- self_comment_0_lines: 5~10개.
- self_comment_6h_lines: 3~10개.
세 배열의 모든 항목은 띄어쓰기와 문장부호를 포함해 10자 이내다.
각 항목에는 빈 문자열이나 줄바꿈을 넣지 않는다.`;
...
export function shouldRegeneratePendingJiwonfit(config, posts) {
  return config?.id === 'jiwonfit'
    && posts.length === 5
    && posts.every((post) => post.status === 'queued')
    && posts.some((post) => !/\n\s*1[.)]\s+/.test(String(post.text || '')));
}
...
async function ensureDailyBatch({ db, client, model, config, now }) {
  ...
  if (shouldRegeneratePendingJiwonfit(config, existing)) {
    const { error: deleteError } = await db
      ...
      .eq('schedule_date', dateKey)
      .eq('status', 'queued');
    if (deleteError) throw new Error(`딱지원핏 예약글 교체 실패: ${deleteError.message}`);
    existing = [];
  }
