...
const APPROVED_QUEUE_MODE = 'approved_queue';
const CAMPAIGN_SLOTS = {
  default: ['08:40', '09:10', '11:50', '12:20', '18:00', '18:30', '20:30', '21:00'],
...
const DEFAULT_EIGHT_POST_PLAN = ['problem', 'tip', 'backstage', 'template', 'problem', 'tip', 'template', 'sale'];
const MAX_CONTENT_LINE_LENGTH = 10;
const CAMPAIGN_MAX_LINE_LENGTH = 60;
const SELF_INTRO_MAX_LINE_LENGTH = 50;
const SELF_INTRO_BODY_LINE_COUNT = 5;
const MAX_POST_LENGTH = 430;
const MAX_THREADS_LENGTH = 500;
const MISSED_SLOT_GRACE_MINUTES = 3;
...
const UNVERIFIED_SOCIAL_PROOF_PATTERNS = [
  /\d+\s*년(?:\s*(?:간|동안))?.{0,20}(?:교육|컨설팅|현장|경력)/,
  /(?:수십|수백|수천)\s*번\s*(?:받|들|보|말|확인)/,
  /(?:사용|이용)\s*후.{0,30}(?:반응|후기|말이|문의)/,
  /(?:가장|제일)\s*(?:많이|자주)\s*(?:받은|들은|나온).{0,20}(?:질문|반응|후기|말)/,
  /(?:고객|사용자|수강생|교육생).{0,25}(?:반응|후기|말했|문의했|요청했)/,
  /(?:교육|컨설팅|현장|운영).{0,30}(?:확인한\s*패턴|가장\s*자주\s*나오|자주\s*(?:받|듣|나오))/,
  /(?:떨어진|탈락한|선정된|합격한)\s*(?:경우|사례).{0,30}(?:보면|대부분|공통)/,
  /운영하면서.{0,20}(?:확인한|알게\s*된|이해하게\s*된)/,
  /오늘.{0,30}(?:봤|보았|확인했|검토했|들었|받았)/,
  /(?:오늘|작업실|상담|첨삭|검토).{0,40}(?:\d+|한|두|세|네|다섯|여섯|일곱|여덟|아홉|열|수십|수백)\s*(?:개|건|명)/,
];
...
const PUBLIC_METRICS_PATTERN = /(?:직전\s*(?:글|게시물)(?:은|의)?\s*)?(?:조회(?:수)?|좋아요|답글|댓글)\s*[:：]?\s*\d+(?:.{0,120}(?:조회(?:수)?|좋아요|답글|댓글)\s*[:：]?\s*\d+)+/s;
const PROFILE_LINK_PATTERN = /프로필\s*링크/;
const COMMENT_PROMOTION_PATTERN = /(?:프로필|링크)/i;
const FEAR_MARKETING_PATTERN = /(?:떨어질|탈락할|불합격할|망할|큰일)/;
...
const CAMPAIGN_CTA_TEMPLATES = {
  default: {
    problem: 'AI가 쓴 티가 나는 문장은 어디인가요?',
    tip: '지금 가장 막힌 자기소개서 항목은 무엇인가요?',
    backstage: '지금 고치고 있는 행동은 무엇인가요?',
    template: '검증이 필요한 회사 정보는 무엇인가요?',
...
const CAMPAIGN_SAFETY_FALLBACKS = {
  default: [
    '확인된 경험과 실제 행동만 근거로 남깁니다.',
    '검증되지 않은 경력이나 결과는 문장에서 제외합니다.',
    '직무 공고와 본인이 확인한 사실만 연결합니다.',
  ],
  jiwonfit: [
    '확인된 공고 원문과 보유 자료만 근거로 남깁니다.',
    '검증되지 않은 선정 사례나 고객 반응은 쓰지 않습니다.',
    '세부 조건은 공고 담당 기관 확인을 기준으로 합니다.',
  ],
};
const SENSITIVE_REPLY_RULES = [
  {
    reason: '자해·극단 선택 암시',
    pattern: /(죽고\s*싶|살고\s*싶지\s*않|사라지고\s*싶|극단적\s*선택|자해|목숨을\s*끊|끝내고\s*싶)/i,
  },
...
const DAILY_BATCH_SCHEMA = {
  ...
  properties: {
    posts: {
      type: 'array',
      description: '캠페인 발행 슬롯 수와 같은 개수의 글',
      items: {
        type: 'object',
        properties: {
          content_type: {
          ...
          text_lines: {
            type: 'array',
            description: 'default는 정확히 5개·50자 이내, jiwonfit은 5~9개·60자 이내의 완결 문장',
            items: { type: 'string' },
          },
          self_comment_0_lines: {
            type: 'array',
            description: '3~8개 완결 문장, 각 항목 60자 이내',
            items: { type: 'string' },
          },
          ...
          search_queries: {
            type: 'array',
            description: '1~4개 검색어, 각 항목 60자 이내',
            items: { type: 'string' },
          },
        },
        required: [
...
export function assertNoPublicPerformanceMetrics(text) {
  const publishableText = String(text || '').trim();
  if (PUBLIC_METRICS_PATTERN.test(publishableText)) {
    throw new Error('직전 글 조회·좋아요·답글 수는 내부 학습용이며 공개 게시할 수 없습니다.');
  }
  return publishableText;
}
...
export function prepareCampaignReply(text, config = {}) {
  const normalized = String(text || '').replace(/\s+/g, ' ').trim();
  if (config.id !== 'unsent_talk') return clampText(normalized, 300);
  const forbidden = [
    LINK_OR_HASHTAG_PATTERN,
    EMOJI_PATTERN,
    /안보낸톡|프로필\s*링크|구매|결제|신청/,
    /(?:해야|하세요|하지\s*마|추천|정답|해결법|연락해|잊어버|시간이\s*해결)/,
    /(?:애착\s*유형|인지\s*왜곡|분리\s*불안|심리\s*분석)/,
  ].some((pattern) => pattern.test(normalized));
  const sentenceCount = normalized.split(/[.!?。！？]+/u).filter(Boolean).length;
  const acceptsOrThanks = /(?:고마|감사|그\s*말|맞는|맞아요|그럴\s*수도|그렇게\s*느낄|생각\s*못|말해줘|남겨줘|읽어줘)/.test(normalized);
  if (!normalized || normalized.length > 60 || sentenceCount > 1 || forbidden || !acceptsOrThanks) {
    return '말해줘서 고마워요.';
  }
  return normalized;
}
...
function engagementTotal(metrics = {}) {
  // Threads의 replies에는 자동 셀프 댓글도 포함되므로 학습 점수에서 뺀다.
  return ['likes', 'reposts', 'quotes', 'shares']
...
export function buildPerformanceLearningContext(posts) {
  const ranked = (posts || [])
    ...
    .map((post) => ({
      ...
      slotIndex: post.slot_index != null && Number.isInteger(Number(post.slot_index))
        ? Number(post.slot_index)
        : null,
      views: Number(post.metrics?.views) || 0,
      engagement: engagementTotal(post.metrics),
      readerReplies: Number(post.reply_count) || 0,
    }))
    .sort((a, b) =>
      b.views - a.views
      || b.engagement - a.engagement
      || b.readerReplies - a.readerReplies)
    .slice(0, 5);
  if (!ranked.length) return '';
  return ranked
    .map((post) =>
      `- ${post.contentType} / 슬롯 ${post.slotIndex ?? '미상'} / "${post.hook}" / 조회 ${post.views} / 독자 반응 ${post.engagement} / 실제 댓글 ${post.readerReplies}`)
    .join('\n');
}

export function contentTypeOrderForDate(dateKey, campaignId = 'default') {
  if (campaignId !== 'default') return [...DAILY_CONTENT_TYPES];
  const match = String(dateKey || '').match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!match) return [...DAILY_CONTENT_TYPES];
  const [, year, month, day] = match;
  const dayIndex = Math.floor(Date.UTC(Number(year), Number(month) - 1, Number(day)) / 86_400_000);
  const saleSlot = ((dayIndex + 1) % DAILY_CONTENT_TYPES.length + DAILY_CONTENT_TYPES.length)
    % DAILY_CONTENT_TYPES.length;
  const saleIndex = DAILY_CONTENT_TYPES.indexOf('sale');
  const rotation = (saleIndex - saleSlot + DAILY_CONTENT_TYPES.length) % DAILY_CONTENT_TYPES.length;
  return [
    ...DAILY_CONTENT_TYPES.slice(rotation),
    ...DAILY_CONTENT_TYPES.slice(0, rotation),
  ];
}

export function contentPlanForDate(dateKey, campaignId = 'default', postCount = DAILY_CONTENT_TYPES.length) {
  if (campaignId === 'default' && postCount === DEFAULT_EIGHT_POST_PLAN.length) {
    const match = String(dateKey || '').match(/^\d{4}-\d{2}-\d{2}$/);
    if (!match) return [...DEFAULT_EIGHT_POST_PLAN];
    const base = [...DEFAULT_EIGHT_POST_PLAN];
    const saleIndex = base.indexOf('sale');
    const desiredSaleIndex = contentTypeOrderForDate(dateKey, campaignId).indexOf('sale');
    [base[saleIndex], base[desiredSaleIndex]] = [base[desiredSaleIndex], base[saleIndex]];
    return base;
  }
  return contentTypeOrderForDate(dateKey, campaignId);
}
...
async function publishText(token, text, replyToId) {
  // 사용자 확정 운영 원칙: 성과 지표는 내부 학습에만 쓰며 모든 공개 게시 경로에서 차단한다.
  const publishableText = assertNoPublicPerformanceMetrics(text);
...
모르는 취업 준비생에게 자기소개서 작성과 검증에 바로 쓰이는 내용을, 취업 강사가 문장 구조와 근거를 검토하듯 공개한다.
...
JSON 스키마 외의 말은 출력하지 않는다.`;
  ...
  const campaignCtaRhythm = config?.id === 'jiwonfit'
    ? '- 하루 5개 CTA는 같은 문구를 반복하지 않는다. 최소 3개는 "프로필 링크"로 다음 행동을 안내하고, 나머지는 관련 경험을 댓글로 묻거나 답글을 유도할 수 있다.'
    : '- 하루 5개 중 가치 제공 4개는 기억할 결론이나 자연스러운 질문으로 끝낸다. "프로필 링크" CTA는 sale 한 편에만 사용한다.';
  const closingLineRhythm = config?.id === 'jiwonfit'
    ? '- 마지막 줄은 "프로필 링크" 또는 "댓글·답글"이라는 목적지와 구체적인 행동 동사를 함께 쓴다.'
    : '- 마지막 줄은 기억할 결론이나 자연스러운 질문을 쓴다. 판매글의 명확한 프로필 링크 안내도 허용한다.';
  const bodyRhythm = config?.id === 'jiwonfit'
    ? `- 본문은 첫 줄 뒤 빈 줄을 두고, 비어 있지 않은 5~9줄과 430자 이내를 유지한다.
- 한 줄은 최대 60자이다.`
    : `- 자소서 계정 본문은 빈 줄 없이 무조건 정확히 5줄로 쓴다. 5~9줄 규칙을 적용하지 않는다.
- 1줄은 익숙한 행동을 뒤집는 문장, 2~4줄은 1.·2.·3. 논리, 5줄은 기억할 결론이나 자연스러운 질문이다.
- 자소서 계정의 한 줄은 공백과 번호를 포함해 최대 50자이다.`;
  const sharedRhythm = `
두 계정 공통의 검증된 글 리듬:
...
${campaignCtaRhythm}
${bodyRhythm}
- 각 줄은 반드시 마침표·물음표·느낌표 중 하나로 끝나는 완결 문장이다. 문장을 여러 줄로 쪼개거나 중간에서 자르지 않는다.
...
- 캠페인 설명에 명시되지 않은 경력 연수, 교육 횟수, 고객 후기, 사용자 반응, 판매 실적을 만들지 않는다.
- 특히 "수십 번 들었다", "사용 후 가장 많이 나온 반응" 같은 사회적 증거는 확인된 입력이 없으면 금지한다.
- "오늘 봤다", "작업실에서 검토했다", "몇 개 중 몇 개였다" 같은 관찰 경험과 수치도 확인된 입력이 없으면 금지한다.
...
function cleanLineItems(lines) {
  return (lines || [])
    ...
    .filter(Boolean)
    .map((line) => {
      if (COMPLETE_ENDING_PATTERN.test(line)) return line;
      return /(?:다|요|함|됨|완료)[’'”"」』)\]]$/u.test(line) ? `${line}.` : line;
    });
}

export function ensureThreeNumberedLogic(lines) {
  if (!Array.isArray(lines) || lines.length < 5) return lines;
  const numbered = [...lines];
  let number = 1;
  for (let index = 1; index < numbered.length - 1; index += 1) {
    const content = String(numbered[index] || '').replace(/^\s*\d+[.)]\s*/, '').trim();
    if (number <= 3) {
      numbered[index] = `${number}. ${content}`;
      number += 1;
    } else {
      numbered[index] = content;
    }
  }
  return numbered;
}
...
function hasUnverifiedSocialProof(text) {
  return UNVERIFIED_SOCIAL_PROOF_PATTERNS.some((pattern) => pattern.test(String(text || '')));
}

function hasFearMarketing(text) {
  return FEAR_MARKETING_PATTERN.test(String(text || ''));
}

export function sanitizeGeneratedLines(lines, {
  comments = false,
  maxLineLength = CAMPAIGN_MAX_LINE_LENGTH,
  safetyFallbacks = CAMPAIGN_SAFETY_FALLBACKS.default,
} = {}) {
  let replacementIndex = 0;
  return (lines || []).map((line) => {
    const normalizedLine = String(line || '').trim();
    const shouldReplace = normalizedLine.length > maxLineLength
      || hasUnverifiedSocialProof(normalizedLine)
      || hasFearMarketing(normalizedLine)
      || (comments && (hasActionableCta(line) || COMMENT_PROMOTION_PATTERN.test(normalizedLine)));
    if (!shouldReplace) return normalizedLine;
    const fallback = safetyFallbacks[replacementIndex % safetyFallbacks.length]
      || '확인된 사실과 자료만 근거로 남깁니다.';
    replacementIndex += 1;
    return fallback;
  });
}
...
export function validateCommentReady(comment, {
  ...
  if (COMMENT_PROMOTION_PATTERN.test(text)) {
    problems.push(`${label}에 프로필 또는 링크 홍보 사용`);
  }
  ...
  if (hasUnverifiedSocialProof(text)) {
    problems.push(`${label}에 근거가 확인되지 않은 경력·후기·고객반응 주장`);
  }
  if (hasFearMarketing(text)) {
    problems.push(`${label}에 공포 마케팅 표현 사용`);
  }
  return problems;
}
...
export function validatePublishReadyPost(post, options = {}) {
  const writingProfile = {
    maxLineLength: CAMPAIGN_MAX_LINE_LENGTH,
    commentMaxLineLength: CAMPAIGN_MAX_LINE_LENGTH,
    minBodyLines: 5,
    maxBodyLines: 9,
    requireBlankLineAfterHook: true,
    allowBlankLines: true,
    requireNumberedPositions: false,
  ...
  if (lines.length < writingProfile.minBodyLines
      || lines.length > writingProfile.maxBodyLines) {
    if (writingProfile.minBodyLines === writingProfile.maxBodyLines) {
      problems.push(`본문이 정확히 ${writingProfile.minBodyLines}줄이 아님`);
    } else {
      problems.push(`본문이 ${writingProfile.minBodyLines}~${writingProfile.maxBodyLines}줄이 아님`);
    }
  }
  if (writingProfile.requireBlankLineAfterHook && !/^[^\n]+\n\s*\n/.test(text)) {
    problems.push('첫 줄 뒤 빈 줄이 없음');
  }
  if (!writingProfile.allowBlankLines
      && text.split('\n').some((line) => !line.trim())) {
    problems.push('본문에 빈 줄 사용');
  }
  ...
  if (writingProfile.requireNumberedStructure) {
    ...
    if (writingProfile.requireNumberedPositions) {
      const numberedPositionsAreValid = [1, 2, 3]
        .every((number, index) => new RegExp(`^${number}[.)]\\s+.+`).test(lines[index + 1] || ''));
      if (!numberedPositionsAreValid) {
        problems.push('본문 2~4줄이 1. 2. 3. 번호 논리가 아님');
      }
    }
  }
  ...
  if (hasUnverifiedSocialProof(text)) {
    problems.push('본문에 근거가 확인되지 않은 경력·후기·고객반응 주장');
  }
  if (hasFearMarketing(text)) {
    problems.push('본문에 공포 마케팅 표현 사용');
  }
  ...
  return [
    ...
    ...validateCommentReady(post?.self_comment_0, {
      label: '첫 댓글',
      maxLineLength: writingProfile.commentMaxLineLength,
    }),
...
function selectExactBodyLines(lines, count) {
  if (!Number.isInteger(count) || count < 2 || lines.length <= count) return lines;
  return [lines[0], ...lines.slice(1, count - 1), lines.at(-1)];
}

export function validateApprovedQueuePost(post) {
  const text = String(post?.text || '').trim();
  const lines = text.split('\n').map((line) => line.trim()).filter(Boolean);
  const problems = [];
  if (!text) problems.push('본문이 비어 있음');
  if (text.length > MAX_THREADS_LENGTH) {
    problems.push(`본문이 ${MAX_THREADS_LENGTH}자를 넘음`);
  }
  if (lines.length < 1 || lines.length > 8) {
    problems.push('본문이 1~8줄이 아님');
  }
  if (LINK_OR_HASHTAG_PATTERN.test(text)) {
    problems.push('본문에 링크 또는 해시태그 사용');
  }
  ...
  const forbidden = [
    ...
    /프로필\s*링크/,
    /(?:구매|결제|신청)(?:하|해|하세요|하기)/,
    /(?:댓글|답글)(?:에|로)?\s*(?:남겨|알려|적어|써|공유|말해|답해)/,
    /(?:애착\s*유형|인지\s*왜곡|분리\s*불안|심리\s*분석|재회\s*(?:상담|강의))/,
  ].find((pattern) => pattern.test(text));
  if (forbidden) {
    problems.push('제품·판매·CTA·심리분석 금지 표현 사용');
  }
  return problems;
}

function orderedDailyPosts(result, writingProfile = {}, contentTypeOrder = DAILY_CONTENT_TYPES) {
  if (!Array.isArray(result.posts) || result.posts.length !== contentTypeOrder.length) {
    throw new Error(`AI가 하루치 글 ${contentTypeOrder.length}개를 반환하지 않았습니다.`);
  }
  const normalized = result.posts.map((post) => {
    const maxLineLength = writingProfile.maxLineLength || MAX_CONTENT_LINE_LENGTH;
    let textLines = writingProfile.strictLineItems
    ...
    if (writingProfile.strictLineItems) {
      textLines = sanitizeGeneratedLines(textLines, writingProfile);
      textLines = selectExactBodyLines(textLines, writingProfile.exactBodyLines);
      textLines = ensureThreeNumberedLogic(textLines);
    }
    ...
    if (writingProfile.strictLineItems) {
      zeroLines = sanitizeGeneratedLines(zeroLines, {
        ...writingProfile,
        comments: true,
        maxLineLength: writingProfile.commentMaxLineLength || CAMPAIGN_MAX_LINE_LENGTH,
      });
    }
    ...
    return {
      ...
      text: writingProfile.compactBody
        ? textLines.join('\n')
        : formatBodyLines(textLines),
  ...
  const ordered = contentTypeOrder.map((type) => {
    const index = remaining.findIndex((post) => post.content_type === type);
    if (index < 0) return null;
    return remaining.splice(index, 1)[0];
  });
  if (ordered.some((post) => !post) || remaining.length) {
    throw new Error(`하루치 콘텐츠 유형이 필요한 배치와 다릅니다: ${contentTypeOrder.join('/')}`);
  }
  return ordered;
}
...
export function validateHumanVoiceBatch(posts, customerPatterns = CUSTOMER_LANGUAGE_PATTERNS, options = {}) {
  ...
  if (profileLinkCtaCount > writingProfile.maxProfileLinkCtas) {
    problems.push(`프로필 링크 CTA가 ${writingProfile.maxProfileLinkCtas}개 초과`);
  }
  return problems;
}

function campaignWritingProfile(config = {}) {
  const isSelfIntroCampaign = config.id !== 'jiwonfit';
  return {
    maxLineLength: isSelfIntroCampaign
      ? SELF_INTRO_MAX_LINE_LENGTH
      : CAMPAIGN_MAX_LINE_LENGTH,
    commentMaxLineLength: CAMPAIGN_MAX_LINE_LENGTH,
    minBodyLines: isSelfIntroCampaign
      ? SELF_INTRO_BODY_LINE_COUNT
      : 5,
    maxBodyLines: isSelfIntroCampaign
      ? SELF_INTRO_BODY_LINE_COUNT
      : 9,
    exactBodyLines: isSelfIntroCampaign
      ? SELF_INTRO_BODY_LINE_COUNT
      : null,
    compactBody: isSelfIntroCampaign,
    requireBlankLineAfterHook: !isSelfIntroCampaign,
    allowBlankLines: !isSelfIntroCampaign,
    requireNumberedPositions: isSelfIntroCampaign,
    requireQuestionCta: false,
    requireNumberedStructure: true,
    minParagraphs: isSelfIntroCampaign ? 1 : 2,
    minZeroCommentLines: 3,
    requireCta: !isSelfIntroCampaign,
    minProfileLinkCtas: config.id === 'jiwonfit' ? 3 : 1,
    maxProfileLinkCtas: config.id === 'jiwonfit' ? 5 : 1,
    strictLineItems: true,
    ctaTemplates: CAMPAIGN_CTA_TEMPLATES[config.id],
    safetyFallbacks: CAMPAIGN_SAFETY_FALLBACKS[config.id]
      || CAMPAIGN_SAFETY_FALLBACKS.default,
  };
}

function campaignContentMode(config = {}) {
  return config.content_mode === APPROVED_QUEUE_MODE
    ? APPROVED_QUEUE_MODE
    : 'generated';
}

export function campaignAutomation(config = {}) {
  const approvedQueue = campaignContentMode(config) === APPROVED_QUEUE_MODE;
  return {
    approvedQueue,
    selfComments: config.auto_self_comments ?? !approvedQueue,
    replies: config.auto_replies ?? !approvedQueue,
    externalComments: config.auto_external_comments ?? !approvedQueue,
    metrics: config.collect_metrics ?? true,
  };
}
...
async function generateDailyBatch(client, model, config, dateKey, performanceContext) {
  ...
  const contentTypeOrder = contentPlanForDate(dateKey, config?.id, slots.length);
  const contentCounts = contentTypeOrder.reduce((counts, type) => ({
    ...counts,
    [type]: (counts[type] || 0) + 1,
  }), {});
  const contentRequirement = Object.entries(contentCounts)
    .map(([type, count]) => `${type} ${count}개`)
    .join(', ');
  const profileLinkRule = config?.id === 'jiwonfit'
    ? '- 5개 중 3개 이상은 마지막 text_lines 항목에 "프로필 링크"를 넣는다. 나머지는 댓글 또는 답글을 자연스럽게 유도한다.'
    : '- default 캠페인은 sale 한 편만 마지막 text_lines 항목에 "프로필 링크"를 넣고, 나머지 4개는 기억할 결론이나 자연스러운 질문으로 끝낸다.';
  const bodyShapeRule = config?.id === 'jiwonfit'
    ? '- text_lines: 5~9개. 첫 항목은 관점 전환 후크, 중간에는 1., 2., 3. 번호 논리를 각각 한 줄의 완결 문장으로 넣고 마지막은 서로 다른 전환 CTA다.'
    : '- text_lines: 무조건 정확히 5개. 1번은 익숙한 행동을 뒤집는 후크, 2~4번은 1., 2., 3. 번호 논리, 5번은 기억할 결론이나 자연스러운 질문이다. 각 항목은 번호와 공백을 포함해 50자 이내다.';
  const itemLengthRule = config?.id === 'jiwonfit'
    ? '본문은 60자, 셀프 댓글은 60자 이내다.'
    : '본문은 50자, 셀프 댓글은 60자 이내다.';
  const outputShape = `각 글은 줄바꿈된 문자열 대신 아래 배열 필드로 작성한다.
${bodyShapeRule}
- self_comment_0_lines: 3~8개.
- self_comment_6h_lines: 3~8개.
${profileLinkRule}
${itemLengthRule} 모든 항목은 마침표·물음표·느낌표 중 하나로 끝나는 자연스러운 완결 문장이다.
...
캠페인 설명에 직접 적힌 근거가 아니면 경력 연수, 교육 횟수, 선정 사례, 사용자 후기나 고객 반응을 만들지 않는다.

오늘의 ${slots.length}개 글이 하나의 작은 연재처럼 이어지되, 각각 단독으로 읽혀야 한다.
content_type은 ${contentRequirement}를 정확히 사용한다.
...
${outputShape}`;
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    ordered = orderedDailyPosts(result, writingProfile, contentTypeOrder);
    voiceProblems = validateHumanVoiceBatch(ordered, customerPatterns, writingProfile);
    if (!voiceProblems.length) break;
    attemptPrompt = `${basePrompt}

이전 결과가 사람냄새·고객언어 검수에서 실패했다. 아래 문제를 모두 고쳐 ${slots.length}개 전체를 다시 작성한다.
...
async function loadConfigs(db, campaignId, { includeDisabled = false } = {}) {
  let query = db
...
async function resolveCampaignToken(db, config, env, { allowRefresh = true } = {}) {
  ...
  if (!allowRefresh || !expiresAt || expiresAt - nowMs >= TOKEN_REFRESH_THRESHOLD_MS) {
...
async function verifyCampaignAccount(token, config) {
  if (!config.profile_username) return null;
...
async function loadRecentPerformanceContext(db, config) {
  const { data, error } = await db
    ...
    .select('content_type,text,slot_index,reply_count,metrics,published_at')
...
export function publishTimesForCampaign(config = {}) {
  if (Array.isArray(config.publish_times) && config.publish_times.length > 0) {
...
async function ensureDailyBatch({ db, client, model, config, now }) {
  ...
  // A persisted daily queue is immutable. Quality validation happens before
  // insertion; maintenance runs must never delete or regenerate approved rows.
  const existing = existingData || [];
  if (campaignContentMode(config) === APPROVED_QUEUE_MODE) return existing;
  ...
  for (const post of posts || []) {
    try {
      ...
      const { data: nextPost } = await db
        ...
        .eq('campaign_id', config.id)
        .eq('sequence', Number(post.sequence) + publishTimesForCampaign(config).length)
        .maybeSingle();
...
async function processPublishing({
  ...
  const automation = campaignAutomation(config);
  const readyStatus = automation.approvedQueue ? 'approved' : 'queued';
  const { data: posts, error } = await db
    .from('threads_autopilot_posts')
    .select('*')
    .eq('campaign_id', config.id)
  ...
  for (const post of posts || []) {
    ...
    try {
      const publishProblems = automation.approvedQueue
        ? validateApprovedQueuePost(post)
        : validatePublishReadyPost(post, campaignWritingProfile(config));
      const forbidden = campaignForbiddenPhrases(config).find((phrase) =>
      ...
      const { error: publishStateError } = await db.from('threads_autopilot_posts').update({
        ...
        reply_due_at: automation.replies ? addMinutes(publishedAt, 5).toISOString() : null,
        reply_done_at: automation.replies ? null : publishedAt.toISOString(),
        six_hour_due_at: automation.selfComments ? addMinutes(publishedAt, 360).toISOString() : null,
        six_hour_done_at: automation.selfComments ? null : publishedAt.toISOString(),
        twenty_four_due_at: automation.metrics ? addMinutes(publishedAt, 1440).toISOString() : null,
        twenty_four_done_at: automation.metrics ? null : publishedAt.toISOString(),
        last_error: null,
      }).eq('id', post.id);
      ...
      await db.from('threads_autopilot_posts').update({
        attempts: Number(post.attempts || 0) + 1,
        status: publishedThreadId || Number(post.attempts || 0) >= 2 ? 'failed' : readyStatus,
  ...
  for (const post of posts || []) {
    try {
      ...
      for (const reply of replies) {
        const responseText = prepareCampaignReply(reply.text, config);
        const responseId = await publishText(token, responseText, reply.target_id);
...
export function threadsAutopilotPaused(env = process.env) {
  return String(env.THREADS_AUTOPILOT_PAUSED || '').trim().toLowerCase() === 'true';
}
...
export async function runThreadsAutopilot({
  ...
  if (threadsAutopilotPaused(env)) {
    return { ok: true, enabled: false, reason: 'paused_by_operator' };
  }
  const configs = await loadConfigs(db, campaignId, { includeDisabled: verifyOnly });
  if (!configs.length) return { ok: true, enabled: false, reason: 'config_missing_or_disabled' };

  if (verifyOnly) {
    if (!campaignId) throw new Error('계정 검증에는 campaign 지정이 필요합니다.');
    const verified = {};
    for (const config of configs) {
      try {
        const token = await resolveCampaignToken(db, config, env, { allowRefresh: false });
        const profile = await verifyCampaignAccount(token, config);
        verified[config.id] = {
          ok: true,
        ...
        verified[config.id] = { ok: false, error: err?.message || String(err) };
      }
    }
    return {
      ok: Object.values(verified).every((item) => item.ok),
      verifyOnly: true,
      campaigns: verified,
    };
  }
  ...
  for (const config of configs) {
    try {
      ...
      const allowExternalComments = automation.externalComments && externalCommentsEnabled(env);
      ...
      const zeroMinuteComments = automation.selfComments
        ? await processZeroComments({ db, token, config })
        : 0;
      ...
      const replyMonitoring = automation.replies
        ? await processContinuousReplies({
            db,
            token,
            client,
            model,
      ...
      campaigns[config.id] = {
        ...
        replyMonitoring,
        sixHourComments: automation.selfComments
          ? await processSixHour({ db, token, config, now })
          : 0,
        twentyFourHourComments: automation.metrics
          ? await processTwentyFourHour({ db, token, config, now })
          : 0,
      };
    } catch (err) {
