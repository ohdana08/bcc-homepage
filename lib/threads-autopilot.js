...
계정 포지션은 "AI 강사의 기관 수주 백스테이지"다.
AI 도구 자랑이 아니라 기관 강의 제안·커리큘럼·견적·운영 업무를 실제로 줄이는 방법을 공개한다.

매일 정확히 5개 글을 쓴다.
1) 문제 인식 2) 실무 팁 3) 제작 백스테이지 4) 템플릿 해부 5) 자연스러운 판매 연결.

규칙:
- 각 본문은 500자 제한을 고려해 450자 안쪽으로 쓴다.
- 첫 문장은 짧고 구체적으로 쓴다.
- 문단을 짧게 나눈다.
- 72시간, 100만원, 무명계정 도전, 매출실험 표현을 사용하지 않는다.
- 확인되지 않은 숫자·성과·계약을 만들지 않는다.
- 본문 5개가 같은 말을 반복하지 않는다.
- 판매글은 하루 1개만 허용한다.
- 상품은 "기관 AI 강의 제안·커리큘럼 실무팩", 가격은 39,900원이다.
- 상품 구성은 편집 가능한 DOCX 20쪽, XLSX 견적·영업관리 계산기, README, LICENSE가 담긴 ZIP이다.
- 구매 링크는 https://www.groble.im/products/KfVDLC 이다.
- self_comment_0은 본문에 없는 구체적 예시나 체크리스트를 추가한다.
- self_comment_6h는 6시간 뒤 붙일 실무 보충 정보다.
- search_queries는 같은 주제의 다른 사람 글을 찾기 위한 짧은 한국어 검색어다.
- 해시태그 도배, 과장, 영혼 없는 공감 문구를 금지한다.
JSON 스키마 외의 말은 출력하지 않는다.`;
}

async function generateDailyBatch(client, model, config, dateKey) {
  const prompt = `작성일: ${dateKey}
캠페인 설명:
...
content_type은 problem, tip, backstage, template, sale을 각각 정확히 한 번 사용한다.`;
  ...
  const ordered = DAILY_CONTENT_TYPES.map((type) => byType.get(type));
...
${candidates.map((item) => `[${item.id}] @${item.username}\n${item.text}`).join('\n\n')}`;
  ...
  const promotional = comments.find((item) =>
    /(https?:\/\/|groble|실무팩|39\s*,?\s*900)/i.test(item.text));
  if (promotional) {
...
상품 질문에는 다음 사실만 사용한다: 기관 AI 강의 제안·커리큘럼 실무팩, 39,900원,
DOCX 20쪽 + XLSX 계산기 + README + LICENSE ZIP,
...
export async function runThreadsAutopilot({
  ...
  const results = {
    twentyFourHour: await processTwentyFourHour({ db, token, now }),
