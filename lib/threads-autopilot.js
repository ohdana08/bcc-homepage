...
export function validateHumanVoiceBatch(posts) {
  ...
  posts.forEach((post, index) => {
    ...
    if (CUSTOMER_LANGUAGE_PATTERNS.some((pattern) => pattern.test(text))) {
...
async function loadRecentPerformanceContext(db) {
  const { data, error } = await db
...
async function ensureDailyBatch({ db, client, model, config, now }) {
  ...
  const performanceContext = await loadRecentPerformanceContext(db);
...
export async function runThreadsAutopilot({
  ...
  const results = {
    ...
    twentyFourHourComments: await processTwentyFourHour({ db, token, now }),
  };
