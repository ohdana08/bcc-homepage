...
const SELF_INTRO_FORBIDDEN_PHRASES = ['자기소개서 워크북', '37쪽 PDF', 'GPT 5개', 'Gem 5개'];
...
const CTA_ACTION_PATTERN = /(?:확인|시작|열어|찾아|눌러|들어|받아|진단|완성|남겨|알려|적어|써|공유|말해|답해|보러)/;
const COMPLETE_ENDING_PATTERN = /[.!?。！？][’'”"」』)\]]*$/u;
const CAMPAIGN_CTA_TEMPLATES = {
  default: {
    sale: '혼자 또 처음부터 쓰지 말고 프로필에서 질문받으며 완성해봐.',
  },
  jiwonfit: {
    ...
    posts: {
      ...
      items: {
        ...
        properties: {
          ...
          text_lines: {
            type: 'array',
            description: '5~9개·60자 이내의 완결 문장',
            items: { type: 'string' },
          },
...
function campaignWritingProfile(config = {}) {
  ...
  return {
    ...
    requireNumberedStructure: !isSelfIntroCampaign,
    minParagraphs: 2,
    minZeroCommentLines: 3,
...
function campaignForbiddenPhrases(config) {
  ...
  return config?.id === 'jiwonfit'
    ? FORBIDDEN_CAMPAIGN_PHRASES
    : [...FORBIDDEN_CAMPAIGN_PHRASES, ...SELF_INTRO_FORBIDDEN_PHRASES];
}
...
async function generateDailyBatch(client, model, config, dateKey, performanceContext) {
  ...
  const bodyShapeRule = config?.id === 'jiwonfit'
    ...
    : `- text_lines: 5~9개. 첫 항목은 취준생이 실제로 겪는 장면이나 단정적인 판단으로 시작한다.
- 중간에는 실제 문장, 전후 비교, 바로 해볼 질문이나 판단 기준 중 하나를 넣는다.
- 1., 2., 3.을 매번 쓰지 않는다. "오늘 댓글을 보니", "핵심은", "세 가지 알려드릴게요" 같은 AI식 도입을 쓰지 않는다.
- sale만 마지막 항목에서 상품 형식 대신 완성되는 결과를 말하며 프로필로 안내한다. 나머지는 글 내용과 연결된 질문이나 행동으로 끝낸다.`;
