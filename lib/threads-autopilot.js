...
- 마지막 줄은 기억할 결론 또는 맥락 있는 전환 CTA다. URL을 직접 쓰지 않는다.
...
- 한 줄은 최대 60자다. 모바일에서 읽기 쉬운 자연스러운 완결 문장으로 쓴다.
- self_comment_0과 self_comment_6h에는 본문에 없는 구체적 예시나 기준을 3~8줄로 보충한다. 댓글에는 CTA나 URL을 넣지 않는다.
...
function orderedDailyPosts(result, writingProfile = {}) {
  ...
  const normalized = result.posts.map((post) => {
    ...
    const textLines = normalizeShortLines(post.text_lines, {
      finalQuestion: Boolean(writingProfile.requireQuestionCta),
      maxLineLength,
    });
    let zeroLines = normalizeShortLines(post.self_comment_0_lines, { maxLineLength });
    if (['tip', 'template'].includes(post.content_type)) {
      zeroLines = ensureNumberedItems(zeroLines, 2, maxLineLength);
    }
    const sixHourLines = normalizeShortLines(post.self_comment_6h_lines, { maxLineLength });
    return {
...
export function validateHumanVoiceBatch(posts, customerPatterns = CUSTOMER_LANGUAGE_PATTERNS, options = {}) {
  ...
  posts.forEach((post, index) => {
    ...
    if (writingProfile.requireCta && !/(프로필\s*링크|댓글|답글|확인해|시작해|열어|알려|[?？]$)/.test(lastLine)) {
      problems.push(`${index + 1}번 본문 마지막에 전환 CTA가 없음`);
    }
    if (writingProfile.requireCta) {
      if (ctas.has(lastLine)) problems.push(`${index + 1}번 CTA가 앞 글과 중복`);
      ctas.add(lastLine);
      if (/프로필\s*링크/.test(lastLine)) profileLinkCtaCount += 1;
    }
...
function campaignWritingProfile() {
  return {
...
- text_lines: 5~9개. 첫 항목은 관점 전환 후크, 중간에는 1., 2., 3. 번호 논리를 넣고 마지막은 서로 다른 전환 CTA다.
  ...
  for (const post of posts || []) {
    ...
    try {
      ...
      if (Number(post.sequence) > 1) {
        ...
        if (previousPost?.status === 'published' && previousPost.thread_id) {
