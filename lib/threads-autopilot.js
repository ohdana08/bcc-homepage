...
const CAMPAIGN_SLOTS = {
  default: ['08:30', '11:30', '14:30', '18:30', '21:30'],
...
const CAMPAIGN_CTA_TEMPLATES = {
  default: {
    problem: '댓글에 AI가 쓴 티가 난 문장 한 줄을 남겨주세요.',
    tip: '댓글에 지금 가장 막힌 자기소개서 항목을 남겨주세요.',
    backstage: '답글에 지금 고치고 있는 행동 한 가지를 적어주세요.',
    ...
    posts: {
      ...
      items: {
        ...
        properties: {
          ...
          text_lines: {
            type: 'array',
            description: '5~9개 완결 문장, 각 항목 57자 이내',
            items: { type: 'string' },
          },
...
export function buildNextPostText(text, metrics = {}) {
  const views = Number(metrics.views) || 0;
  const likes = Number(metrics.likes) || 0;
  const replies = Number(metrics.replies) || 0;
  const intro = `직전 글은 조회 ${views} · 좋아요 ${likes} · 답글 ${replies}였어.`;
  return clampText(`${intro}\n\n${String(text || '').trim()}`, MAX_THREADS_LENGTH);
}
...
JSON 스키마 외의 말은 출력하지 않는다.`;
  ...
  const campaignCtaRhythm = config?.id === 'jiwonfit'
    ...
    : '- 하루 5개 중 가치 제공 4개는 댓글·답글 질문으로 끝낸다. "프로필 링크" CTA는 sale 한 편에만 사용한다.';
  const sharedRhythm = `
두 계정 공통의 검증된 글 리듬:
...
async function generateDailyBatch(client, model, config, dateKey, performanceContext) {
  const profileLinkRule = config?.id === 'jiwonfit'
    ...
    : '- default 캠페인은 sale 한 편만 마지막 text_lines 항목에 "프로필 링크"를 넣고, 나머지 4개는 댓글 또는 답글을 자연스럽게 유도한다.';
...
async function processPublishing({
  ...
  for (const post of posts || []) {
    ...
    try {
      ...
      let publishTextValue = post.text;
      const { data: previousPost, error: previousPostError } = await db
        .from('threads_autopilot_posts')
        .select('id,thread_id,status')
        .eq('campaign_id', config.id)
        .eq('status', 'published')
        .lt('sequence', Number(post.sequence))
        .not('thread_id', 'is', null)
        .order('sequence', { ascending: false })
        .limit(1)
        .maybeSingle();
      if (previousPostError) {
        throw new Error(`직전 글 조회 실패: ${previousPostError.message}`);
      }
      if (previousPost?.thread_id) {
        const previousMetrics = await getPostInsights(token, previousPost.thread_id);
        const { error: metricsSaveError } = await db
          .from('threads_autopilot_posts')
          .update({
        ...
        if (metricsSaveError) {
          throw new Error(`직전 글 성과 저장 실패: ${metricsSaveError.message}`);
        }
        publishTextValue = buildNextPostText(post.text, previousMetrics);
      }
