...
export function findClaude101LessonAsset(lessonId) {
  return CLAUDE101_COURSE_ASSETS.find((lesson) => lesson.id === lessonId) || null;
}

export function listClaude101LessonAssetPaths(lessonId) {
  const lesson = findClaude101LessonAsset(lessonId);
  if (!lesson) return [];
  return [
    lesson.videoPath,
    lesson.subtitlePath,
...
export async function createClaude101SignedLessonAssets(db, bucket, lessonId, expiresIn = 3600) {
  const lesson = findClaude101LessonAsset(lessonId);
  if (!lesson || !bucket) return null;

  const { data, error } = await db.storage
    .from(bucket)
    .createSignedUrls(listClaude101LessonAssetPaths(lessonId), expiresIn);
  if (error || !Array.isArray(data)) return null;

  const urls = new Map(data.map((item) => [item.path, item.signedUrl || item.url || null]));
  return {
    id: lesson.id,
    videoUrl: urls.get(lesson.videoPath) || null,
    subtitleUrl: urls.get(lesson.subtitlePath) || null,
