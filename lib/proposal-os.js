...
const PROPOSAL_SCHEMA_GROUPS = [
  {
    ...
    maxTokens: 4000,
    instruction: '먼저 기관 요구를 분석하고 적합한 과정과 무료 도구 인계 흐름을 추천한다.',
  },
  {
    keys: ['proposal', 'curriculum'],
    maxTokens: 6000,
    instruction: '확정된 요구분석과 추천안을 바탕으로 제안서 본문과 실제 운영 가능한 커리큘럼을 작성한다.',
  },
  {
    keys: ['email', 'qa'],
    maxTokens: 3500,
    instruction: '앞서 작성된 제안 내용을 바탕으로 기관 회신 이메일 초안과 최종 검수 결과를 작성한다.',
  },
];
...
async function generateStructuredGroup(client, { keys, maxTokens, instruction, userContent }) {
  ...
  if (msg.stop_reason === 'max_tokens') {
    throw Object.assign(new Error('제안서 생성 분량이 한도를 넘었습니다. 내용을 줄여 다시 시도해 주세요.'), { httpStatus: 502 });
  }
