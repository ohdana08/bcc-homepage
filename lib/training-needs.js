...
const TOOL_ACCESS_OPTIONS = [
  ...
  ['other:used', '기타 도구 · 직접 입력', 'other', 'used', '기타 도구'],
];
...
export function resolvePublicFormReference(input = {}) {
  return clean(input.form || input.code, 20).toUpperCase();
}
...
export function summarizeSurveyList(surveys = []) {
  const actualSurveys = surveys.filter((survey) => !survey?.is_sample);
  return {
    open: actualSurveys.filter((survey) => survey.status === 'open').length,
    responses: actualSurveys.reduce((sum, survey) => sum + Number(survey.response_count || 0), 0),
    analyzed: actualSurveys.filter((survey) => survey.analysis_generated_at).length,
    total: actualSurveys.length,
    samples: surveys.length - actualSurveys.length,
  };
}
...
function cleanSurveyInput(input = {}) {
  ...
  row.case_id = input.case_id && /^[0-9a-f-]{36}$/i.test(String(input.case_id)) ? String(input.case_id) : null;
  return row;
}

function cleanSurveyChanges(input = {}) {
  const source = input && typeof input === 'object' ? input : {};
  const changes = {};
  const detailKeys = ['title', 'institution_label', 'audience_label', 'lecture_date', 'expected_response_count', 'case_id'];
  const hasDetails = detailKeys.some((key) => key in source);
  if (hasDetails) Object.assign(changes, cleanSurveyInput(source));
  if (source.status && ['draft', 'open', 'closed', 'analyzed'].includes(source.status)) changes.status = source.status;
  if (!hasDetails && 'response_deadline' in source) {
    const value = String(source.response_deadline || '');
    changes.response_deadline = /^\d{4}-\d{2}-\d{2}$/.test(value) ? value : null;
  }
  if (Object.keys(changes).length === 0) throw Object.assign(new Error('수정할 내용을 입력해 주세요.'), { httpStatus: 400 });
  return changes;
}

async function publicSurvey(db, reference) {
  const publicCode = clean(reference, 20).toUpperCase();
  if (!publicCode) throw Object.assign(new Error('기관에서 받은 전용 설문 링크로 접속해 주세요.'), { httpStatus: 400 });
  const { data, error } = await db
    .from('training_need_surveys')
    .select('id, public_code, title, institution_label, audience_label, lecture_date, response_deadline, expected_response_count, status, is_sample')
    .eq('public_code', publicCode)
    .maybeSingle();
  if (error) throw error;
  if (!data) throw Object.assign(new Error('전용 설문 링크를 확인해 주세요.'), { httpStatus: 404 });
...
export async function handleTrainingNeedsPublic(req, res, { db }) {
  ...
  if (action === 'get') {
    const survey = await publicSurvey(db, resolvePublicFormReference(req.body));
    const { id: _id, public_code: _publicCode, expected_response_count: _expected, ...publicData } = survey;
...
async function listSurveys(db) {
  ...
  return {
    ...
    metrics: summarizeSurveyList(surveysWithCounts),
...
async function createSurvey(db, user, input) {
  ...
  throw new Error('전용 설문 링크를 생성하지 못했습니다. 다시 시도해 주세요.');
}
...
export async function handleTrainingNeedsAdmin(req, res, { db, client }) {
  ...
  if (action === 'update') {
    ...
    if (!id) return res.status(400).json({ error: '수정할 수요조사를 선택해 주세요.' });
    const changes = cleanSurveyChanges(req.body?.changes);
